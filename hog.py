...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 2
    turn_score, max_score = 0, 25 - num_rolls
    if num_rolls == 0:
...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while scores[0] < goal and scores[1] < goal:
        ...
        if scores[player] == 2 * scores[other(player)] or 2*scores[player] == scores[other(player)]:
            temp = scores[player]
            scores[player] = scores[other(player)]
            scores[other(player)] = temp
        player = other(player)
...
def final_strategy(score, opponent_score):
    ...
    if score > 90:
        n = 3
    return swap_strategy(score, opponent_score, margin, n)
